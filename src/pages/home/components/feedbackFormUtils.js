...
export const getSimilarRoutes = async (text) => {
  try {
    const response = await fetch(`http://localhost:3000/suggest?text=${encodeURIComponent(text)}`);
...
export const updateRouteInfo = (multiRouteRef, myMapRef, balloonRef, routeData, setRouteData) => {
  ...
  if (referencePoints.length >= 2) {
    ...
    const routeInfo = `Время в пути: ${hours} ч ${minutes} мин, Расстояние: ${newRouteData.distance.toFixed(2)} км, Стоимость: ${cost} руб`;
...
export const prepareFormData = (data, routeData) => {
  const tripDateTimeISO = data.tripDate && data.tripTime 
    ? new Date(`${data.tripDate}T${data.tripTime}:00`).toISOString()
    : null;
...
export const submitForm = async (formData) => {
  const response = await fetch('http://localhost:3000/telegram/notify', {
    method: 'POST',
    headers: {
